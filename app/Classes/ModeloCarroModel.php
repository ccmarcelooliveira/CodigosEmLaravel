 ...
 * @author Celia Regina
 */
class ModeloCarroModel {
    
    public function ConsultaModeloCarro($idMarca) {
        
        //echo $idPlanoContas;
        //GRADE DE SEGURANCA
        $seguranca = new \App\Http\Controllers\Master\ControleSistema\SegurancaController();
        $idMarca_decripto = $seguranca -> decripto($idMarca); 
           

        $results = DB::table('modelo_carros')      
                               ->select('modelo_carros.*')                    
                               ->where('modelo_carros.CmpStatus', '=', 'ATV')                                  
                               ->where('modelo_carros.fabricante_idfabricante', '=', $idMarca_decripto)                                 
                               ->orderBy('modelo_carros.CmpModelo')
                               ->get();
